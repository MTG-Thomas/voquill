...
function renderItemText(item: HistoryItem, searchQuery: string, isRaw: boolean) {
  ...
  if (!text.trim()) {
    const placeholder =
      item.error_message ||
      (item.status === "empty" ? "No speech detected" : "Transcription failed");
    return (
      <span style={{ color: tokens.colors.textMuted, fontStyle: "italic" }}>{placeholder}</span>
    );
  }
        ...
        >
          {displayItems.length === 0 ? (
            ...
            displayItems.map((item) => {
              ...
              const displayText =
                isRaw && item.raw_text ? item.raw_text : item.text || item.error_message || "";
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <div
                      style={{ fontSize: tokens.typography.sizeXs, color: tokens.colors.textMuted }}
                    >
