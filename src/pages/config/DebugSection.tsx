...
export function DebugSection({
  ...
  return (
    <>
      <ConfigField
        label="Session Logging Level"
        description="Control how detailed log entries are in the diagnostic log."
      >
      ...
      <ConfigField
        label="Save Recording Logs"
        description="Save raw audio from recordings and file imports as WAV files in the debug folder for troubleshooting."
      >
        <div
          style={{
            display: "flex",
      ...
      <ConfigField
        label="History Limit"
        description="Maximum number of history entries to keep. Oldest entries are automatically pruned when the limit is exceeded. 0 = unlimited."
      >
        <NumberField
          value={config.history_limit}
          onChange={(value) => updateConfig("history_limit", value)}
          min={0}
          max={10000}
        />
      </ConfigField>
      ...
      <ConfigField
        label="Unload Model"
        description="Free GPU/CPU memory by unloading the transcription model from memory. It will reload automatically on next dictation."
      >
        <Button
