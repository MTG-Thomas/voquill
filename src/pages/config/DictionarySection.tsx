      ...
      >
        <div style={{ display: "flex", gap: tokens.spacing.xs, width: "100%" }}>
          <input
            ...
            onInput={(e) => {
              dictionaryInput.value = (e.target as HTMLInputElement).value;
            }}
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                addWord();
          ...
          />
          <Button variant="configAction" onClick={addWord} disabled={!dictionaryInput.value.trim()}>
            Add
          </Button>
        </div>
        {(config.dictionary || []).length > 0 && (
          <div
            style={{ display: "flex", flexWrap: "wrap", gap: "4px", marginTop: tokens.spacing.xs }}
          >
