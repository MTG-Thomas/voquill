  ...
  return (
    <>
        ...
        <>
              ...
              <SelectField
                ...
                onChange={(nextEngine) => {
                  ...
                  if (modelsForEngine.length > 0) {
                    const recommended =
                      modelsForEngine.find((m) => m.recommended) || modelsForEngine[0];
          ...
          {config.local_engine.includes("(GPU)") && gpuStatus?.tested && !gpuStatus.available && (
            ...
            >
              ...
              <span>
                GPU unavailable:{" "}
                {gpuStatus.detail ||
                  "No compatible GPU detected. Select Whisper.cpp (CPU) or check your GPU drivers."}
              </span>
            </div>
          )}
          ...
          {engineCapabilities && engineCapabilities.settings.length > 0 && (
            <ConfigField
              ...
              description="Fine-tune how this engine processes your speech."
            >
              <EngineSettingsPanel
      ...
      <ConfigField
        ...
        description="Best-effort language hint for transcription models. Helps with spelling preferences (e.g. Australian, British, American English) and non-English dictation."
      >
        <div style={selectWrapperStyle}>
          <SelectField
      ...
      <ConfigField
        label="Differentiate Voices in Recordings"
        description="Detect and label different speakers in live recordings. When enabled, the recording takes slightly longer to process as each speaker segment is transcribed independently."
      >
        <Switch
          name="Diarization Recording"
          checked={config.diarization_enabled_recording}
          onChange={(checked) => updateConfig("diarization_enabled_recording", checked)}
        />
      </ConfigField>
      ...
      {(config.diarization_enabled_recording || config.diarization_enabled_files) && (
        <ConfigField
          ...
          description="Higher values merge similar voices (fewer speaker labels); lower values detect more distinct voices. Adjust if you're seeing too many or too few speakers."
        >
          <div
            style={{
              display: "flex",
      ...
      <ConfigField
        label="Post-roll (ms)"
        description="Extra audio (in milliseconds) captured after releasing the hotkey. Helps prevent the last sentence from being cut off, especially with API models."
      >
        <NumberField
      ...
      <ConfigField
        ...
        description="Recording automatically stops and transcribes after this many minutes (1-180)."
      >
        <NumberField
