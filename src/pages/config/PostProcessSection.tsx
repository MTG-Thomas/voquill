...
const DEFAULT_POST_PROCESS_PROMPT =
  ...
  return (
    <>
      <ConfigField
        ...
        description="Run transcribed text through a language model to fix punctuation, capitalization, and remove filler words."
      >
        <Switch
          name="Post-Processing"
          checked={config.post_process_enabled}
          onChange={(checked) => updateConfig("post_process_enabled", checked)}
        />
      </ConfigField>
      ...
      {config.post_process_enabled && (
        <>
          <ConfigField
            label="Method"
            description="Choose between a local model or a cloud API for post-processing."
          >
            <ModeSwitcher
          ...
          {config.post_process_provider === "API" ? (
            <>
              <ConfigField
                label="API Key"
                description="Used to authenticate with the post-processing service (OpenAI, OpenRouter, etc.)."
              >
                <div style={{ ...selectWrapperStyle }}>
                  <input
                    ...
                    onChange={(e: Event) =>
                      updateConfig("post_process_api_key", (e.target as HTMLInputElement).value)
                    }
                    placeholder="sk-..."
                  />
                  <Button variant="configAction" onClick={testCleanupApi}>
                    Test
                  </Button>
                </div>
              </ConfigField>
              ...
              <ConfigField
                label="API URL"
                description="The OpenAI-compatible endpoint for post-processing (e.g. OpenRouter, local llama-server)."
              >
                <input
                  style={inputBaseStyle}
                  type="url"
                  ...
                  onChange={(e: Event) =>
                    updateConfig("post_process_api_url", (e.target as HTMLInputElement).value)
                  }
              ...
              <ConfigField
                label="Model"
                description="The model name to use with your post-processing API provider."
              >
                <input
                  style={inputBaseStyle}
                  type="text"
                  ...
                  onChange={(e: Event) =>
                    updateConfig("post_process_api_model", (e.target as HTMLInputElement).value)
                  }
            ...
            <>
              <ConfigField
                label="Local Engine"
                description="CPU works everywhere. GPU (Vulkan) accelerates post-processing on compatible hardware."
              >
                <div style={selectWrapperStyle}>
                  <SelectField
              ...
              {config.post_process_engine.includes("(GPU)") &&
                postProcessGpuStatus?.tested &&
                !postProcessGpuStatus.available && (
                  <div
                    style={{
                      display: "flex",
                      alignItems: "center",
                  ...
                  >
                    ...
                    <span>
                      GPU unavailable:{" "}
                      {postProcessGpuStatus.detail ||
                        "No compatible GPU detected. Using the CPU engine instead."}
                    </span>
                  </div>
                )}
              ...
              <ConfigField
                label="Local Model"
                description="Download a GGUF model for on-device post-processing. No internet connection needed after download."
              >
                <ModelSelectionPanel
                  availableModels={(availableModels || []).filter(
                    (m) => m.engine === config.post_process_engine,
                  )}
          ...
          <ConfigField
            label="System Prompt"
            description="The system prompt sent to the post-processing model. Customize how your text is cleaned. You can create multiple prompts and switch between them."
          >
            <div
              style={{
                display: "flex",
            ...
            >
              ...
              >
                <div style={{ ...selectWrapperStyle, flex: 1, minWidth: 0 }}>
                  <SelectField
                    ...
                    options={[
                      ...
                      ...(config.post_process_prompts || []).map((p) => ({
                        value: p.id,
                        label: p.name,
                      })),
                    ]}
                ...
                >
                  <Button
                    ...
                    onClick={() => {
                      const name =
                        ...
                        `Prompt ${(config.post_process_prompts || []).length + 1}`;
                      const id = `custom_${Date.now()}`;
                      const prompts = [
                        ...(config.post_process_prompts || []),
                        {
                          id,
                          name,
                          prompt: config.post_process_prompt || DEFAULT_POST_PROCESS_PROMPT,
                        },
                      ];
                  ...
                  {config.post_process_selected_prompt_id &&
                    (config.post_process_prompts || []).length > 0 && (
                      <Button
                        variant="danger"
                        pill
                        style={configGhostPillStyle}
                        onClick={() => {
                          const prompts = (config.post_process_prompts || []).filter(
                            (p) => p.id !== config.post_process_selected_prompt_id,
                          );
                          updateConfig("post_process_prompts", prompts);
              ...
              <textarea
                ...
                value={(() => {
                  ...
                  if (selectedId) {
                    const found = (config.post_process_prompts || []).find(
                      (p) => p.id === selectedId,
                    );
          ...
          <ConfigField
            ...
            description="How your transcript is wrapped before sending to the model. Use {transcript} as the placeholder for your dictated text."
          >
            <div
              style={{
                display: "flex",
              ...
              <textarea
                ...
                value={(() => {
                  ...
                  if (selectedId) {
                    ...
                    if (
                      found &&
                      found.user_prompt_template !== undefined &&
                      found.user_prompt_template !== null
                    )
                      return found.user_prompt_template;
                  }
          ...
          <ConfigField
            ...
            description="Maximum number of tokens the post-processing model can generate. 0 = auto (scales with transcript length, max 8192)."
          >
            <NumberField
              value={(() => {
                const selectedId = config.post_process_selected_prompt_id;
                if (selectedId) {
                  ...
                  if (
                    found &&
                    found.max_output_tokens !== undefined &&
                    found.max_output_tokens !== null
                  )
