  ...
  return (
    <>
      <ConfigField
        ...
        description="Automatically remove filler words (uh, umm, hmm, etc.) from transcriptions using a fast regex pass. Works without post-processing."
      >
        <Switch
      ...
      <ConfigField
        ...
        description="Add words to also remove. Built-in words (uh, umm, hmm, etc.) are always removed. Add extra words like 'literally', 'actually', 'basically' if you tend to overuse them."
      >
        <div
          style={{
            display: "flex",
        ...
        >
          <div style={{ display: "flex", gap: tokens.spacing.xs, width: "100%" }}>
            <input
              ...
              onInput={(e) => {
                fillerWordInput.value = (e.target as HTMLInputElement).value;
              }}
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  addFillerWord();
