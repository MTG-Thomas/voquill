import { IconCheck, IconInfoCircle, IconRefresh, IconShieldLock } from "@tabler/icons-preact";
    ...
    <div style={{ ...tabPanelStyle, overflow: "auto" }} key="initial-setup">
      <div style={{ ...tabPanelPaddedStyle, flex: 1 }}>
        ...
        >
          ...
          >
            Complete these required checks before first use:
          </p>
          <div>
            <div
              style={{
            ...
            <SettingRow
              ...
              status={
                permissions?.shortcuts && !hotkeyError ? (
                  <IconCheck color="var(--colors-success)" size={20} />
                ) : (
                  <Button
                    variant="ghost"
            ...
            >
              {!permissions?.shortcuts && !isSystemManagedShortcut && (
                <input
                  ...
                  title={
                    portalVersion >= 1 ? "Use Configure Hotkey to request a system shortcut." : ""
                  }
                />
              )}
            ...
            <SettingRow
              ...
              status={
                readiness.isTranscriptionReady ? (
                  <IconCheck color="var(--colors-success)" size={20} />
                ) : null
              }
            >
              {config.transcription_mode === "Local" ? (
                  ...
                  >
                    <SelectField
                      ...
                      options={availableEngines.map((engine) => ({
                        value: engine,
                        label: engine.includes("(GPU)")
                          ? `${engine} — Recommended for dedicated GPUs`
                          : engine,
                      }))}
                      style={{ minWidth: 0 }}
                    />
                    <div style={helperTextStyle}>
                      GPU engines transcribe on your graphics card — dramatically faster than CPU
                  ...
                  {isGpuEngineSelected &&
                    ...
                    !gpuStatus.available && (
                      <div style={warningTextStyle}>
                        GPU unavailable: {gpuStatus.detail || "No compatible GPU detected."} Voquill
                        will fall back to the CPU engine automatically.
                      </div>
                    )}
                ...
                <div style={selectWrapperStyle}>
                  <input
                    ...
                    type="text"
                    value={
                      config.openai_api_key === API_KEY_PLACEHOLDER ? "" : config.openai_api_key
                    }
            ...
            >
              ...
              {readiness.audioDeviceIssue !== "no-devices" && (
                <SelectField
                  ...
                  options={availableMics.map((mic) => ({
                    value: mic.id,
                    label: mic.label || mic.id,
                  }))}
        ...
        >
          ...
          {!readiness.isAllReady && setupTouched && (
            <div
              style={{ marginTop: "8px", fontSize: "11px", color: "#d9dfe7", textAlign: "center" }}
            >
