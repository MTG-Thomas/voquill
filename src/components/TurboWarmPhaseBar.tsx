...
function getPhaseIndex(startedAt?: number | null) {
  ...
  return Math.min(
    Math.floor(elapsedMs / TURBO_WARM_PHASE_MS),
    turboWarmPhases.length - 1,
  );
}
...
export function TurboWarmPhaseBar({ compact = false, startedAt = null }: TurboWarmPhaseBarProps) {
  ...
  return (
    <div style={{ width: compact ? '138px' : '260px', display: 'flex', flexDirection: 'column', gap: compact ? '4px' : '6px' }}>
      <style>{`
      ...
      `}</style>
      {!compact && (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: tokens.spacing.sm }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: compact ? '3px' : '5px' }}>
        {turboWarmPhases.map((phase, index) => (
          <span
            ...
            style={{
              ...
              animation: index === phaseIndex ? 'voquill-turbo-dot-pulse 1.1s ease-in-out infinite' : 'none',
            }}
          ></span>
        ))}
      </div>
