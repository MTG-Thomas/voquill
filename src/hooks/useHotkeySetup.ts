...
export function useHotkeySetup(options: UseHotkeySetupOptions): UseHotkeySetupReturn {
  ...
  useEffect(() => {
    ...
    invoke<OverlayPositioningCapabilities>("get_overlay_positioning_capabilities")
      .then((v) => {
        overlayPositioningCapabilities.value = v;
      })
      .catch((e) => {
  ...
  return {
    ...
    setHotkeyBindingState: (state) => {
      hotkeyBindingState.value = state;
    },
    setSystemShortcutContext: (context) => {
      systemShortcutContext.value = context;
    },
    setShowHotkeyCaptureModal: (show) => {
      showHotkeyCaptureModal.value = show;
    },
    setShowSystemShortcutModal: (show) => {
      showSystemShortcutModal.value = show;
    },
    setShowFactoryResetModal: (show) => {
      showFactoryResetModal.value = show;
    },
    setIsApplyingHotkey: (applying) => {
      isApplyingHotkey.value = applying;
    },
